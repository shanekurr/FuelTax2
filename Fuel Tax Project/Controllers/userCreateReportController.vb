    ...
    Public Class userCreateReportController
        ...
        Function ContentUpload_Post(ed1 As FileData) As ActionResult
            ...
            Else
                ...
                Dim tempPath1 As String = "~/Reports/" & "FYR/" & year & "/"
                Dim tempPath2 As String = "~/Reports/" & "GAS/" & year & "/"

                ' store the file inside ~/Reports/Newreport         
                path_1 = Path.Combine(Server.MapPath(tempPath1), filename)
                path_2 = Path.Combine(Server.MapPath(tempPath2), filename2)
                ...
                If Not Directory.Exists(Server.MapPath(tempPath1)) Then
                    Directory.CreateDirectory(Server.MapPath(tempPath1))
                End If
                If Not Directory.Exists(Server.MapPath(tempPath2)) Then
                    Directory.CreateDirectory(Server.MapPath(tempPath2))
                End If
                'save file to server
                pf.SaveAs(path_1)
                pf2.SaveAs(path_2)
            ...
            Dim months As Array = {"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"}
            Dim currentMonth As String = month

            'here we will create 12 reports , one for each month, but to the user it will look as if we cretaed only one report for the month they selected
            For Each month In months
                ...
                Try
                    ...
                    If Not Directory.Exists(Server.MapPath("~/Reports/NEW/" & year & "/" & month & "/")) Then
                        Directory.CreateDirectory(Server.MapPath("~/Reports/NEW/" & year & "/" & month & "/"))
                    End If

                    'saves a copy of the workbook to the server
                    oWB.SaveCopyAs(Server.MapPath("~/Reports/NEW/" & year & "/" & month & "/" & year & month & ".xlsx"))
                    ...
                    If currentMonth = month Then
                        ...
                        Response.TransmitFile(Server.MapPath("~/Reports/NEW/" & year & "/" & month & "/" & downfname))
                        Response.[End]()
                    End If
